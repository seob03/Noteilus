...
class PdfController {
  ...
  async getSummary(req, res) {
    try {
      console.log('AI 요약 요청 시작 - PDF ID:', req.params.pdfId);
      ...
      if (existingSummary && existingSummary.summary) {
        console.log('기존 저장된 요약 발견 - DB에서 반환');
        return res.json({ 
          success: true, 
      ...
      console.log('새로운 요약 생성 시작 - OCR 텍스트 길이:', pdf.ocrText.length);
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: '당신은 문서 정리 전문가입니다. 주어진 문서를 깔끔하고 자세하고 명확하게 정리해주세요.'
            },
            {
              role: 'user',
              content: `
              Role (역할): 당신은 전문 문서 요약 정리 전문가이자 정보 구조화 전문가입니다. 제공될 OCR 마크다운 텍스트를 분석하여, 원본 문서의 핵심 내용을 정확하고 고급스럽게 정리하되, 
              특히 학습 목적에 불필요한 서론, 결론, 강의 목차, 날짜, 과정 안내 등 부수적인 정보는 과감히 제외하고 순수한 학습 내용만을 추출하여 가독성 높은 마크다운 형식으로 재구성하는 데 탁월한 능력을 가지고 있습니다.
              Context (맥락): 저는 PDF 문서에서 OCR을 통해 추출된 마크다운 형식의 텍스트를 제공할 것입니다. 이 텍스트는 원본 문서의 구조를 최대한 반영하고 있습니다. 저는 이 원본 텍스트에서 오직 핵심 학습 내용만을 구조적으로 이해하고, 미려한 형식으로 정리된 요약본을 한국어로 받고 싶습니다. 불필요한 정보를 걸러내고 순도 높은 학습 자료를 얻는 것이 주된 목표입니다.
              Objective (목표): 제공된 OCR 마크다운 텍스트를 고급지고, 자세하며, 가독성 높은 정리본으로 만들어주세요. 특히 원본 문서의 핵심 학습 내용과 논리적 구조를 최대한 보존하면서 정리하는 것이 중요합니다. 시험이나 학습에 직접적으로 관련 없는 부분(예: 강의 소개, 강사 정보, 강의 날짜, 프로그램 일정, 불필요한 서론 및 결론 등)은 절대 포함하지 말고, 학습 내용만을 선별하여 정리리해주세요.
              ...
              Response Format (응답 형식): 결과물은 다음 마크다운(Markdown) 형식의 지침을 엄격히 준수하여 작성해 주세요:
                  • 제외할 부분: 학습 내용과 직접 관련 없는 서론, 결론, 강의 목차, 날짜, 프로그램 안내, 강사 소개, 불필요한 예시의 반복과 ![img-1.jpeg](img-1.jpeg)과 같은 이미지 파일 등은 요약본에서 완전히 배제하고, 핵심 정보 위주로 빠르게 확인할 수 있도록 해 주세요.
                  • 제목 및 소제목: 원본 문서의 목차나 섹션처럼 보이는 핵심 학습 내용 부분은 #, ##, ### 등의 적절한 헤딩(Heading) 레벨을 사용하여 계층 구조를 명확히 표현해 주세요. 더 나은 가독성을 위해 헤딩별로 구분을 위한 적절한 줄바꿈을 사용해 주세요.
                  • 핵심 키워드 강조: 볼드체( **텍스트** )를 사용하여 명확하게 강조해 주세요. 이때 백틱을 사용해서 인라인 블록 방식으로 작성해주세요.
                  • 공식 및 수식: 문서 내에 등장하는 공식이나 수학적 표현은 LaTeX 문법을 사용하여 깔끔하게 렌더링될 수 있도록 해주세요. 블록 수식의 경우 $$수식$$ 형식으로, 인라인 수식의 경우 $수식$ 형식으로 직접 작성해주세요. 백틱을 사용하지 마세요. 특히 수식 내의 특수문자나 기호는 LaTeX 문법에 맞게 정확히 변환해주세요.
                  • 코드 블록: 만약 원본 텍스트에 프로그래밍 코드가 포함되어 있다면, 해당 코드를 언어 (예: python)로 시작하는 코드 블록 내에 정확히 삽입해 주세요. 이는 코드 가독성을 높이고 AI가 코드를 명확히 구분하여 처리하도록 합니다.
                  • 리스트: 내용 정리 시 **순서가 있는 리스트(Numbered List)**나 **순서 없는 리스트(Bullet List)**를 적절히 활용하여 가독성을 높여주세요. 특히 넘버링 리스트는 작업의 순서나 우선순위를 제시할 때, 불릿 리스트는 복잡한 요구사항을 항목별로 명확히 제시할 때 유용합니다.
                  • 구조 유지: 원본 텍스트의 논리적 흐름과 섹션별 구조를 최대한 보존하면서 요약해 주세요.
                  • 불필요한 설명 생략: 학습 내용과 직접 관련 없는 서론, 결론, 강의 목차, 날짜, 프로그램 안내, 강사 소개, 불필요한 예시의 반복 등은 요약본에서 완전히 배제하고, 핵심 정보 위주로 빠르게 확인할 수 있도록 해 주세요.
                  • 분량: 요약본의 전체 길이는 원본 텍스트의 핵심 정보와 중요도를 고려하되, 필수적인 학습 정보를 충분히 담을 수 있도록 상세하게 작성해 주세요.
      ...
      if (!response.ok) {
        ...
        return res.status(500).json({ error: 'AI 요약 처리에 실패했습니다.' });
      }

      const result = await response.json();
      const summary = result.choices[0].message.content;

      console.log('AI 요약 완료 - 길이:', summary.length);
      ...
      res.status(500).json({ error: 'AI 요약 처리에 실패했습니다.' });
    }
  }
  ...
  async getTranslation(req, res) {
    try {
      ...
      // 1단계: 요약본 확인 (프론트엔드에서 전달받은 요약본 우선 사용)
      console.log('1단계: 요약본 확인');
      let summary;

      if (sourceContent && sourceContent.trim() !== '') {
        console.log('프론트엔드에서 전달받은 요약본 사용');
        summary = sourceContent;
      } else {
        // 프론트엔드에서 요약본을 전달하지 않은 경우, DB에서 기존 요약본 확인
        console.log('프론트엔드 요약본 없음 - DB에서 기존 요약본 확인');
        let existingSummary = await this.pdfDocument.getAISummary(pdfId);
        
        if (existingSummary && existingSummary.summary) {
          console.log('기존 요약본 발견 - 재사용');
          summary = existingSummary.summary;
        } else {
          console.log('기존 요약본 없음 - 새로 생성');
          const summaryResponse = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: 'system',
                  content: '당신은 문서 요약 전문가입니다. 주어진 문서를 깔끔하고 명확하게 요약해주세요.'
                },
                {
                  role: 'user',
                  content: `Role (역할): 당신은 전문 문서 요약가이자 정보 구조화 전문가입니다. 
                  OCR을 통해 변환된 마크다운 텍스트를 분석하고, 핵심 내용을 정확하고 고급스럽게 요약하여 가독성 높은 마크다운 형식으로 재구성하는 데 탁월한 능력을 가지고 있습니다.
                  Context (맥락): 저는 PDF 문서에서 OCR을 통해 추출된 마크다운 형식의 텍스트를 제공할 것입니다. 이 텍스트는 원본 문서의 구조를 최대한 반영하고 있습니다. 저는 이 원본 텍스트의 핵심 내용을 구조적으로 이해하고, 미려한 형식으로 정리된 요약본을 받고 싶습니다.
                  Objective (목표): 제공된 OCR 마크다운 텍스트를 고급지고, 자세하며, 가독성 높은 요약본으로 만들어주세요. 특히 원본 문서의 핵심 내용과 논리적 구조를 최대한 보존하면서 요약하는 것이 중요합니다.
                  Style (스타일): 요약본은 전문적이면서도 명료한 문체를 사용해야 합니다. 원본 내용의 의도를 정확하게 전달하면서도, 불필요한 장황함 없이 간결하게 표현해 주세요. 정보의 계층적 중요도를 시각적으로 명확하게 나타내어 주세요.
                  Tone (어조): 정보 전달에 중점을 둔 객관적이고 중립적인 어조를 유지해 주세요. 독자가 내용을 쉽게 신뢰하고 집중할 수 있도록 전문성을 담아 전달해 주세요.
                  Audience (독자): 이 요약본은 해당 분야에 대한 기본적인 이해는 있으나, 원본 문서 전체를 읽을 시간이 부족한 전문가나 학습자를 대상으로 합니다.
                  Response Format (응답 형식): 결과물은 다음 마크다운(Markdown) 형식의 지침을 엄격히 준수하여 작성해 주세요:
                  • 제목 및 소제목: 원본 문서의 목차나 섹션처럼 보이는 부분은 #, ##, ### 등의 적절한 헤딩(Heading) 레벨을 사용하여 계층 구조를 명확히 표현해 주세요. 이는 GPT가 문서를 논리적 계층 구조로 이해하는 데 도움을 줍니다.
                  • 핵심 키워드 강조: 각 섹션의 중요한 개념이나 키워드는 백틱을 사용하여 강조해 주세요. 이를 통해 GPT가 특정 용어를 핵심 포인트로 인식하게 됩니다.
                  • 공식 및 수식: 문서 내에 등장하는 공식이나 수학적 표현은 LaTeX 문법을 사용하여 깔끔하게 렌더링될 수 있도록 해주세요. 블록 수식의 경우 $$수식$$ 형식으로, 인라인 수식의 경우 $수식$ 형식으로 직접 작성해주세요. 백틱을 사용하지 마세요. 특히 수식 내의 특수문자나 기호는 LaTeX 문법에 맞게 정확히 변환해주세요.
                  • 코드 블록: 만약 원본 텍스트에 프로그래밍 코드가 포함되어 있다면, 해당 코드를 언어 (예: python)로 시작하는 코드 블록 내에 정확히 삽입해 주세요. 이는 코드 가독성을 높이고 AI가 코드를 명확히 구분하여 처리하도록 합니다.
                  • 리스트: 내용 정리 시 **순서가 있는 리스트(Numbered List)**나 **순서 없는 리스트(Bullet List)**를 적절히 활용하여 가독성을 높여주세요. 특히 넘버링 리스트는 작업의 순서나 우선순위를 제시할 때, 불릿 리스트는 복잡한 요구사항을 항목별로 명확히 제시할 때 유용합니다.
                  • 강조: 특정 내용의 중요도를 부각하고 싶을 때는 볼드체( **텍스트** )나 이탤릭체( *텍스트* )를 사용하여 명확하게 강조해 주세요. 이를 통해 GPT가 강조된 용어를 핵심 포인트로 인식합니다.
                  • 구조 유지: 원본 텍스트의 논리적 흐름과 섹션별 구조를 최대한 보존하면서 요약해 주세요. 이는 GPT가 문서를 논리적 계층 구조로 이해하고 해당 영역에 적합한 응답을 생성하는 데 도움이 됩니다.
                  • 분량: 요약본의 전체 길이는 원본 텍스트의 분량과 중요도를 고려하되, 핵심 정보를 충분히 담을 수 있도록 상세하게 작성해 주세요.
                  • 불필요한 설명 생략: 서론과 결론 등 불필요한 맥락 설명을 생략하고 핵심 정보 위주로 빠르게 확인할 수 있도록 해 주세요.
                  • 요약해야할 OCR markdown 텍스트 : ${pdf.ocrText}`
                }
              ],
              max_tokens: 1000,
              temperature: 0.3
          ...
          if (!summaryResponse.ok) {
            ...
            console.error('요약본 생성 실패:', summaryResponse.status, errorText);
            return res.status(500).json({ error: 'AI 요약 처리에 실패했습니다.' });
          }

          const summaryResult = await summaryResponse.json();
          summary = summaryResult.choices[0].message.content;
          console.log('요약본 생성 완료 - 길이:', summary.length);
          
          // 새로 생성된 요약본을 DB에 저장
          await this.pdfDocument.saveAISummary(pdfId, summary);
          console.log('요약본 DB 저장 완료');
        }
      }

      // 2단계: 요약본을 번역
      console.log('2단계: 요약본 번역 시작');
      const translationResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: '당신은 전문 번역가입니다. 주어진 마크다운 형식의 텍스트를 자연스럽고 정확하게 번역해주세요. 마크다운 문법은 그대로 유지하면서 내용만 번역해주세요. 특히 LaTeX 수식($$...$$ 또는 $...$)은 그대로 유지하고 번역하지 마세요.'
            },
            {
              role: 'user',
              content: `다음 마크다운 형식의 요약본을 ${targetLanguage}로 번역해주세요. 마크다운 문법(#, *, **, \` 등)은 그대로 유지하고 내용만 번역해주세요. LaTeX 수식($$...$$ 또는 $...$)은 번역하지 말고 그대로 유지해주세요:\n\n${summary}`
            }
          ],
          max_tokens: 2000,
          temperature: 0.3
